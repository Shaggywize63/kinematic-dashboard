...
function HRSection() {
  ...
  const FField = (id: keyof typeof emptyForm, label: string, type='text', opts?: {v:string;l:string}[]) => (
    <div key={id}>
      ...
      {opts ? (
        <select
          style={{ width:'100%', padding:'9px 12px', borderRadius:10, border:`1.5px solid ${C.border}`, background:C.s3, color:C.white, fontSize:13, fontFamily:"'DM Sans',sans-serif", outline:'none', colorScheme:'dark' }}
        ...
        <>
          ...
          <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
            <div style={{ position:'relative', flex:1, minWidth:200 }}>
              ...
              <input
                placeholder="Search by name, ID, mobile, city…"
            ...
            <div style={{ display:'flex', gap:6 }}>
              {['all','executive','admin','supervisor'].map(r=>(
                <button key={r} onClick={()=>setRoleFilter(r)} style={roleBtnStyle(r)}>
          ...
          {loading ? (
            ...
            <Card><div style={{ textAlign:'center', padding:'32px 0', color:C.grayd, fontSize:13 }}>No users found{search ? ` for "${search}"` : ''}</div></Card>
          ) : (
            <Card style={{ padding:0, overflow:'hidden' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'2.2fr 1.2fr 1.4fr 1.2fr 1fr 110px',
                ...
                {['Name / ID','Role','Mobile','Zone / City','Status','Actions'].map(h=>(
                  <span key={h}>{h}</span>
                ))}
              </div>
              {/* Rows */}
          ...
          <Card>
            ...
            {loading ? <div style={{ display:'flex', justifyContent:'center', padding:40 }}><Spin/></div> : (
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                {[
                  { l:'Field Executives', v:executives.length,  c:C.blue,   total:users.length },
                  { l:'Admins',           v:admins.length,       c:C.red,    total:users.length },
                  { l:'Supervisors',      v:supervisors.length,  c:C.teal,   total:users.length },
                  { l:'Other',            v:users.filter(u=>!['executive','admin','supervisor'].includes(u.role)).length, c:C.purple, total:users.length },
                ].map((x,i)=>(
                  <div key={i}>
                    ...
                    <div style={{ height:4, background:C.s3, borderRadius:2, overflow:'hidden' }}>
                      <div style={{ height:'100%', background:x.c, borderRadius:2, width:`${x.total>0?(x.v/x.total)*100:0}%`, transition:'width .8s ease' }}/>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {[
            ...
            <div key={i} style={{ display:'flex', gap:14, padding:'14px 0', borderBottom:i<3?`1px solid ${C.border}`:'none', alignItems:'center' }}>
              <div style={{ width:40, height:40, borderRadius:12,
                background:m.type==='Video'?C.redD:C.blueD,
        ...
        <div style={modalOverlay} onClick={e=>e.target===e.currentTarget&&setShowReset(false)}>
          <div style={{ ...modalBox, maxWidth:380 }}>
            ...
            <div style={{ display:'flex', gap:10, padding:'11px 14px', background:C.yellowD, border:`1px solid ${C.yellow}28`, borderRadius:10, marginBottom:16 }}>
              ...
              <span style={{ fontSize:12, color:C.yellow, lineHeight:1.5 }}>The user will be required to log in with this new password immediately.</span>
            </div>
            <div>
...
export default function DashboardPage() {
  ...
  // ── view toggle: 'dashboard' | 'hr'
  const [view, setView] = useState<'dashboard'|'hr'>('dashboard');
  ...
  return (
    <>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:22, animation:'km-fadein .3s ease' }}>
        ...
        {/* ── TOP NAV: Dashboard / HR toggle ── */}
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', flexWrap:'wrap', gap:12 }}>
          <div style={{ display:'flex', alignItems:'center', gap:0, background:C.s2, border:`1px solid ${C.border}`, borderRadius:12, padding:3 }}>
            {([
              ...
              <button key={v.id} onClick={()=>setView(v.id)}
                style={{ padding:'8px 18px', borderRadius:10, border:'none',
                  background: view===v.id ? C.red : 'transparent',
                  color: view===v.id ? '#fff' : C.gray,
                  fontSize:13, fontWeight:700, cursor:'pointer',
                  fontFamily:"'DM Sans',sans-serif", transition:'all .18s',
                  boxShadow: view===v.id ? `0 4px 14px ${C.redB}` : 'none' }}>
                {v.label}
              </button>
            ))}
          </div>
