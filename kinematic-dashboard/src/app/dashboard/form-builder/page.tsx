          ...
          {shown.map(f => (
            <div key={f.id} onClick={() => onOpen(f)}
              ...
              <div style={{ padding:'16px 18px' }}>
                ...
                {f.description && <div style={{ fontSize:12, color:C.gray, marginBottom:10, lineHeight:1.5, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical' as any, overflow:'hidden' }}>{f.description}</div>}
...
function FormEditor({ form: initialForm, onBack }:{ form:BForm; onBack:()=>void }) {
  ...
  const loadForm = useCallback(async () => {
    try {
      ...
      if (pList.length === 0) {
        ...
        setPages(Array.isArray(pList)?pList.sort((a,b) => a.page_order - b.page_order):[]);
  ...
  useEffect(() => {
    ...
    apiFetch<any>(`/api/v1/builder/forms/${form.id}/submissions`)
      .then(d => { const list = Array.isArray(d)?d:(d?.data??[]); setSubs(Array.isArray(list)?list:[]); })
      .catch(()=>setSubs([]))
  ...
  const addPage = async () => {
    const newPage = await apiFetch<BPage>(`/api/v1/builder/forms/${form.id}/pages`, {
      method:'POST', body: JSON.stringify({ title:`Page ${(pages?.length??0)+1}`, page_order:pages.length })
    });
      ...
      {tab==='submissions' && (
        <div style={{ flex:1, overflowY:'auto', padding:'28px 40px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
            ...
            <div style={{ fontSize:13, color:C.gray }}>{subs?.length ?? 0} total</div>
          </div>
          {loadingSubs ? (
