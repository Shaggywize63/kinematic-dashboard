...
export const mockRoutePlans = () => ({
  ...
  data: [
    { id: 'rp1', date: new Date().toISOString().split('T')[0], executive: { name: 'Arjun Sharma' }, status: 'active', stores_count: 8, progress: 4,
      vehicle_type: '2w_petrol', co2_kg_planned: 1.94, co2_kg_actual: 1.12 },
    { id: 'rp2', date: new Date().toISOString().split('T')[0], executive: { name: 'Priya Patel'  }, status: 'active', stores_count: 6, progress: 2,
      vehicle_type: '4w_diesel', co2_kg_planned: 3.42, co2_kg_actual: 1.71 }
  ]
});

const buildEsgDays = (n: number) => {
  const today = new Date();
  const out: { day: string; co2_kg: number }[] = [];
  for (let i = n - 1; i >= 0; i--) {
    const d = new Date(today.getTime() - i * 86400000);
    out.push({ day: d.toISOString().slice(0, 10), co2_kg: +(2 + Math.sin(i / 2) * 1.4 + (i % 3) * 0.4).toFixed(2) });
  }
  return out;
};

export const mockEsgSummary = () => {
  const days = buildEsgDays(14);
  const totalActual = +days.reduce((s, d) => s + d.co2_kg, 0).toFixed(2);
  const totalPlanned = +(totalActual * 1.08).toFixed(2);
  return {
    success: true,
    data: {
      ...
      total_km: +(totalActual / 0.072).toFixed(1),
      delta_vs_planned_pct: +(((totalActual - totalPlanned) / totalPlanned) * 100).toFixed(2),
      by_vehicle: {
        '2w_petrol': { km: +((totalActual * 0.55) / 0.072).toFixed(1), co2_kg: +(totalActual * 0.55).toFixed(2), plan_count: 9 },
        '4w_diesel': { km: +((totalActual * 0.30) / 0.171).toFixed(1), co2_kg: +(totalActual * 0.30).toFixed(2), plan_count: 4 },
        '2w_ev':     { km: +((totalActual * 0.15) / 0.022).toFixed(1), co2_kg: +(totalActual * 0.15).toFixed(2), plan_count: 3 },
      },
      daily_series: days,
      equivalents: {
        trees_year: +(totalActual / 21).toFixed(2),
        home_days:  +(totalActual / 4.5).toFixed(2),
      },
      plan_count: 16,
    },
  };
...
export const mockOptimizeRoute = (outlets: { id: string }[] = []) => ({
  success: true,
  data: {
...
export function matchDemoMock<T>(rawPath: string, method: string, body?: unknown): T | undefined {
  ...
  if (m === 'GET') {
    ...
    if (path === '/route-plans/esg-summary' || path.startsWith('/route-plans/esg-summary?')) return mockEsgSummary() as unknown as T;
  ...
  if (m === 'POST' || m === 'PATCH' || m === 'PUT') {
    ...
    if (m === 'POST' && (path === '/route-plans/optimize' || path === '/route-plans/optimize/me')) {
      const outlets = (bodyObj.outlets as { id: string }[] | undefined) || [];
      return mockOptimizeRoute(outlets) as unknown as T;
    }
