'use client';
/**
 * WhatsApp Campaigns Access — super-admin control for the broadcast entitlement
 * (paid add-on). Lists every org's on/off state and toggles it via
 * PUT /api/v1/admin/broadcast-entitlement/:orgId. Backed by the central
 * broadcast_orgs table; the /crm/broadcasts mutation routes read the same flag,
 * so a change here immediately governs who can run campaigns.
 */
import { useEffect, useState, useCallback } from 'react';
import { broadcastEntitlementApi, type BroadcastOrg } from '../../../../lib/crmApi';
...
export default function WhatsappCampaignsAccessPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await broadcastEntitlementApi.list();
  ...
  const setEnabled = async (org: BroadcastOrg, enabled: boolean) => {
    ...
    try {
      await broadcastEntitlementApi.set(org.org_id, { enabled });
    } catch (e: any) {
  ...
  const addOrg = async () => {
    ...
    setSaving(true);
    try {
      await broadcastEntitlementApi.set(orgId, { enabled: addForm.enabled, label: addForm.label.trim(), project_key: addForm.project_key.trim() || null });
      setShowAdd(false);
  ...
  if (!isSuperAdmin) {
    return <div style={{ ...card, maxWidth: 560, margin: '40px auto', textAlign: 'center', color: C.gray, fontSize: 14 }}>Only platform super-admins can manage WhatsApp Campaigns access.</div>;
  }
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", color: C.white, paddingBottom: 40, maxWidth: 820 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, gap: 12, flexWrap: 'wrap' }}>
        <div>
          ...
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: 24, fontWeight: 800, margin: '6px 0 4px' }}>WhatsApp Campaigns Access</h1>
          <p style={{ color: C.gray, fontSize: 13, margin: 0, maxWidth: 620 }}>
            Control which organisations can run WhatsApp broadcast campaigns. It&apos;s a paid add-on — <b style={{ color: C.white }}>off by default</b>. Enabling an org unlocks the CRM → Campaigns section and its send routes.
          </p>
        </div>
