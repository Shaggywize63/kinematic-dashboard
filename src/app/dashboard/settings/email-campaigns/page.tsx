'use client';
/**
 * Email Campaigns Access — super-admin control for the bulk-email entitlement
 * (paid add-on). Lists every org's on/off state and toggles it via
 * PUT /api/v1/admin/email-campaign-entitlement/:orgId. Backed by the central
 * email_campaign_orgs table; the /crm/email-campaigns mutation routes read the
 * same flag, so a change here immediately governs who can run email campaigns.
 */
import { useEffect, useState, useCallback } from 'react';
import { emailCampaignEntitlementApi, type BroadcastOrg } from '../../../../lib/crmApi';
...
export default function EmailCampaignsAccessPage() {
  const [rows, setRows] = useState<BroadcastOrg[]>([]);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState<Record<string, boolean>>({});
  const [error, setError] = useState('');
  const [showAdd, setShowAdd] = useState(false);
  const [addForm, setAddForm] = useState({ org_id: '', label: '', project_key: 'kinematic', enabled: true });
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await emailCampaignEntitlementApi.list();
      setRows((r?.data ?? []) as BroadcastOrg[]);
      setError('');
  ...
  const setEnabled = async (org: BroadcastOrg, enabled: boolean) => {
    setBusy((b) => ({ ...b, [org.org_id]: true }));
    setRows((rs) => rs.map((r) => r.org_id === org.org_id ? { ...r, enabled } : r));
    try {
      await emailCampaignEntitlementApi.set(org.org_id, { enabled });
    } catch (e: any) {
  ...
  const addOrg = async () => {
    ...
    setSaving(true);
    try {
      await emailCampaignEntitlementApi.set(orgId, { enabled: addForm.enabled, label: addForm.label.trim(), project_key: addForm.project_key.trim() || null });
      setShowAdd(false);
      setAddForm({ org_id: '', label: '', project_key: 'kinematic', enabled: true });
      load();
  ...
  if (!isSuperAdmin) {
    return <div style={{ ...card, maxWidth: 560, margin: '40px auto', textAlign: 'center', color: C.gray, fontSize: 14 }}>Only platform super-admins can manage Email Campaigns access.</div>;
  }
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", color: C.white, paddingBottom: 40, maxWidth: 820 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, gap: 12, flexWrap: 'wrap' }}>
        <div>
          ...
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: 24, fontWeight: 800, margin: '6px 0 4px' }}>Email Campaigns Access</h1>
          <p style={{ color: C.gray, fontSize: 13, margin: 0, maxWidth: 620 }}>
            Control which organisations can run bulk email campaigns. It&apos;s a paid add-on — <b style={{ color: C.white }}>off by default</b>. Enabling an org unlocks the CRM → Email Campaigns section and its send routes.
          </p>
        </div>
        <button onClick={() => setShowAdd((v) => !v)} style={{ background: C.s3, border: `1px solid ${C.border}`, color: C.white, padding: '9px 14px', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>{showAdd ? 'Close' : '+ Add organisation'}</button>
      </div>

      {showAdd && (
        <div style={{ ...card, marginBottom: 16 }}>
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 15, fontWeight: 800, marginBottom: 4 }}>Add an organisation</div>
          <div style={{ fontSize: 12, color: C.gray, marginBottom: 16 }}>Enter the organisation&apos;s id (UUID) and its Supabase project key (e.g. <code>kinematic</code> or <code>default</code>).</div>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12, marginBottom: 12 }}>
            <div><span style={label}>Label</span><input value={addForm.label} onChange={(e) => setAddForm({ ...addForm, label: e.target.value })} placeholder="e.g. Acme Corp" style={input} /></div>
            <div><span style={label}>Project key</span><input value={addForm.project_key} onChange={(e) => setAddForm({ ...addForm, project_key: e.target.value })} placeholder="kinematic" style={input} /></div>
            <div style={{ gridColumn: '1 / -1' }}><span style={label}>Organisation id (UUID)</span><input value={addForm.org_id} onChange={(e) => setAddForm({ ...addForm, org_id: e.target.value })} placeholder="00000000-0000-0000-0000-000000000000" style={{ ...input, fontFamily: 'monospace' }} /></div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: C.gray, cursor: 'pointer' }}>
              <input type="checkbox" checked={addForm.enabled} onChange={(e) => setAddForm({ ...addForm, enabled: e.target.checked })} style={{ accentColor: C.green }} /> Enable immediately
            </label>
        ...
        {loading ? (
          ...
          <div key={r.org_id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 20px', borderBottom: i < rows.length - 1 ? `1px solid ${C.border}` : 'none' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: C.gray, marginTop: 3, fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.org_id}{r.project_key ? ` · ${r.project_key}` : ''}</div>
            </div>
      ...
      <div style={{ fontSize: 12, color: C.gray, marginTop: 14, lineHeight: 1.6 }}>Changes take effect within a minute. Disabling an org blocks new campaigns and every send action immediately.</div>
    </div>
  );
}
