...
export default function ZoneManagement() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{show:boolean; item:Zone|null}>({show:false, item:null});
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/api/v1/zones/${deleteConfirm.item.id}`);
    ...
    <div>
        ...
        : filtered.map((z,i)=>(
          ...
          >
            ...
            <div style={{display:'flex',gap:6}}>
              ...
              <button onClick={()=>toggle(z)} title={z.is_active?'Deactivate':'Activate'} style={{width:30,height:30,border:`1px solid ${C.border}`,borderRadius:8,background:'transparent',cursor:'pointer',color:z.is_active?C.red:C.green,display:'flex',alignItems:'center',justifyContent:'center'}}
              ...
              {isPlatformAdmin && (
                <button onClick={()=>setDeleteConfirm({show:true, item:z})} title="Delete Zone" style={{width:30,height:30,border:`1px solid ${C.border}`,borderRadius:8,background:'transparent',cursor:'pointer',color:C.gray,display:'flex',alignItems:'center',justifyContent:'center'}}
      ...
      {showModal && (
        <Overlay onClose={()=>setShowModal(false)}>
          <div style={{background:C.s2,border:`1px solid ${C.border}`,borderRadius:20,padding:28,width:'100%',maxWidth:520,maxHeight:'90vh',overflowY:'auto',position:'relative'}}>
            {/* Close Button (X) */}
