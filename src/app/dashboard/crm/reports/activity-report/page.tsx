...
export default function ActivityReportPage() {
  ...
  return (
    <div style={{ background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: 14, padding: 18 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-dim)', marginTop: 4, maxWidth: 560 }}>
            Every logged activity with lead, directory (dealer) and notes — one row per activity. Runs for
            the last week by default; change the range and press Run, or download the full CSV.
          </div>
        </div>
      ...
      <ReportRunner endpointPath="/api/v1/crm/activities/export-activity-report" filenameBase="activity-report" />
    </div>
  );
}
