...
export default function LocationsSettingsPage() {
  ...
  const loadStates = async () => {
    ...
    try {
      const r = await crmStatesApi.list({ limit: 200 });
      setStates(r.data || []);
  ...
  const loadCities = async (stateId: string) => {
    ...
    try {
      const r = await crmStatesApi.cities(stateId);
      setCities(r.data || []);
  ...
  const seedIndian = async () => {
    setSeeding(true);
    try {
      const r = await crmStatesApi.seedIndian();
      toast.success(`Seeded ${r.data.states} states / ${r.data.cities} cities`);
      await loadStates();
  ...
  const addState = async (e: React.FormEvent) => {
    ...
    try {
      await crmStatesApi.create({ name: newStateName.trim(), code: newStateCode.trim() || null, country: 'India' });
      setNewStateName(''); setNewStateCode('');
      await loadStates();
  ...
  const deleteState = async (s: CrmState) => {
    ...
    try {
      await crmStatesApi.remove(s.id);
  ...
  const toggleStateActive = async (s: CrmState) => {
    try {
      await crmStatesApi.update(s.id, { is_active: !s.is_active });
      await loadStates();
  ...
  const addCity = async (e: React.FormEvent) => {
    ...
    try {
      await crmCitiesApi.create({ state_id: selectedState.id, name: newCityName.trim() });
      setNewCityName('');
  ...
  const deleteCity = async (c: CrmCity) => {
    ...
    try {
      await crmCitiesApi.remove(c.id);
  ...
  const toggleCityActive = async (c: CrmCity) => {
    try {
      await crmCitiesApi.update(c.id, { is_active: !c.is_active });
      if (selectedState) await loadCities(selectedState.id);
    } catch (e: any) { toast.error(e.message || 'Update failed'); }
  };
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        ...
        <button
          onClick={seedIndian}
          disabled={seeding}
          style={{ background: 'var(--primary)', border: 'none', color: '#fff', padding: '8px 14px', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer', opacity: seeding ? 0.6 : 1 }}
        >
          {seeding ? 'Seeding…' : 'Seed Indian Locations (36 states / 140+ cities)'}
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr', gap: 14 }}>
        ...
        <div style={{ background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: 14, padding: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--text)' }}>States ({states.length})</div>
          </div>
          ...
          {loadingStates ? (
            ...
            <div style={{ color: 'var(--text-dim)', fontSize: 13, padding: '14px 0' }}>
              No states yet. Click <strong>Seed Indian Locations</strong> above for a quick start.
            </div>
          ) : (
              ...
              {states.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); toggleStateActive(s); }}
                    ...
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); deleteState(s); }}
                      title="Delete"
                      style={iconBtn(active)}
                    >
                      🗑
        ...
        <div style={{ background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: 14, padding: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--text)', marginBottom: 12 }}>
            {selectedState ? `Cities in ${selectedState.name} (${cities.length})` : 'Cities'}
          </div>

          {!selectedState ? (
            <div style={{ color: 'var(--text-dim)', fontSize: 13 }}>Pick a state on the left to see and manage its cities.</div>
          ) : (
            <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 480, overflowY: 'auto' }}>
                  {cities.map((c) => (
                    <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 8, background: 'var(--s3)', opacity: c.is_active ? 1 : 0.55 }}>
                      ...
                      <button type="button" onClick={() => toggleCityActive(c)} title={c.is_active ? 'Deactivate' : 'Activate'} style={iconBtn(false)}>{c.is_active ? '✓' : '×'}</button>
                      <button type="button" onClick={() => deleteCity(c)} title="Delete" style={iconBtn(false)}>🗑</button>
                    </div>
                  ))}
                </div>
              )}
...
function iconBtn(onPrimary: boolean): React.CSSProperties {
  return {
    background: 'transparent',
    border: 'none',
    color: onPrimary ? 'rgba(255,255,255,0.85)' : 'var(--text-dim)',
    cursor: 'pointer',
    fontSize: 13,
    padding: '2px 6px',
    borderRadius: 4,
