...
export default function DealEditModal({ deal, stages, open, onClose, onSaved }: Props) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const r = await crmDeals.update(deal.id, { name: form.name, amount: form.amount ? Number(form.amount) : 0, stage_id: form.stage_id, probability: form.probability ? Number(form.probability) / 100 : null, expected_close_date: form.expected_close_date || null, next_action: form.next_action || null });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Edit Deal"
      ...
      <div style={{ marginTop: 14 }}><span style={lbl}>Next Action</span><input value={form.next_action} onChange={(e) => setForm({ ...form, next_action: e.target.value })} style={{ ...inp, width: '100%', marginTop: 4 }} /></div>
    </Modal>
  );
}

function seed(d: Deal) { return { name: d.name || '', amount: d.amount ? String(d.amount) : '', stage_id: d.stage_id || '', probability: d.probability != null ? String(Math.round(d.probability * 100)) : '', expected_close_date: d.expected_close_date ? d.expected_close_date.slice(0, 10) : '', next_action: d.next_action || '' }; }
