'use client';
/**
 * Reusable Sales Leaderboard.
 *
 * Used in two places:
 *   - CRM Overview / dashboard page (compact mode: top 10, header-only
 *     controls, links to the full view).
 *   - Standalone /dashboard/crm/leaderboard page (full mode: top 50,
 *     all controls, summary cards).
 *
 * Behaviour notes (carried over from the original standalone page):
 *  - Metric (Count vs Revenue) is persisted to localStorage so a manager
 *    who prefers revenue doesn't have to re-pick it every visit.
 *  - Period MTD / QTD / YTD / Custom is local state; the standalone page
 *    intentionally doesn't reuse the global DateRangePicker store because
 *    its presets are tighter than 7d/30d/90d.
 *  - Top-3 ranks get medal emojis; the rest are plain numerals.
 *  - Both Count and Revenue columns are always rendered; the metric
 *    toggle re-sorts and highlights the primary one.
 */
...
export interface SalesLeaderboardProps {
  /**
   * Compact mode (for embedding on the CRM Overview):
   *  - Caps rows to `limit` (default 10).
   *  - Hides summary cards and the period segmented selector (uses MTD).
   *  - Adds a "See full leaderboard →" footer link.
   *
   * Full mode (standalone page):
   *  - Caps rows to `limit` (default 50).
   *  - Shows all controls and summary cards.
   */
  compact?: boolean;
  limit?: number;
}

export default function SalesLeaderboard({ compact = false, limit }: SalesLeaderboardProps) {
  const rowCap = limit ?? (compact ? 10 : 50);
  ...
  // useState's initialiser) so SSR markup stays stable and we don't trip
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await crmLeaderboard.get({
        metric,
        period,
  ...
  const rows = useMemo(() => allRows.slice(0, rowCap), [allRows, rowCap]);
  const totalCount = useMemo(() => allRows.reduce((s, r) => s + r.count, 0), [allRows]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: compact ? 16 : 20, fontWeight: 800, color: 'var(--text)' }}>
            🏆 Top Reps
          </h2>
          <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--text-dim)' }}>
            {compact ? 'Top performers by closed-won ' : 'Top reps by closed-won '}
            {metric === 'revenue' ? 'revenue' : 'deal count'}
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          {/* Period segmented — hidden in compact mode (we lock to MTD for
              the overview embed so the home page stays focused). */}
          {!compact && (
            <div style={{ display: 'inline-flex', background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: 10, padding: 3 }}>
              {PERIODS.map((p) => (
                <button
      ...
      {/* Summary cards — only in full mode. Compact mode keeps the home
          page tight; managers can drill into the full view for totals. */}
      {!compact && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
          <div style={{ background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: 12, padding: 14 }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--text)', marginTop: 4 }}>{allRows.length}</div>
          </div>
          <div style={{ background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: 12, padding: 14 }}>
      ...
      {/* Compact-mode footer link to the full leaderboard. We only show this
          when there's data to drill into; on empty/loading we keep the card
          tight. */}
      {compact && !loading && allRows.length > 0 && (
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <Link
            href="/dashboard/crm/leaderboard"
            style={{
              fontSize: 12,
              fontWeight: 700,
              color: 'var(--primary)',
