 ...
 * Recharts visual.
 *
 * Used inside react-grid-layout cells on /dashboard/crm/leads/analytics
 * and on the CRM Overview where users pin widgets.
 */
...
import { Settings2, X, Pin, MoreVertical, AlertTriangle } from 'lucide-react';
import { crmAnalyticsExt, crmDashboardLayouts, type WidgetInstance } from '../../../lib/crmAnalyticsExtApi';
...
interface Props {
  ...
  onRemove?: (id: string) => void;       // shown when editing the analytics page
  onPinToOverview?: (w: WidgetInstance) => void;
  onChangeChartType?: (id: string, chartType: ChartType) => void;
  isEditing?: boolean;
  pinned?: boolean;                       // hide the "Pin" action on the overview itself
}

export default function AnalyticsWidget({ widget, onRemove, onPinToOverview, onChangeChartType, isEditing = false, pinned = false }: Props) {
  ...
  if (!meta) {
    return <Tile title="Unknown widget"><div style={empty}>Widget type not recognised</div></Tile>;
  }
  ...
  return (
    <div style={card}>
      ...
      <div style={header}>
        ...
        {/* Hover menu — only when editing or pinning */}
              ...
              {meta.supportedCharts.length > 1 && (
                <>
                  ...
                  {meta.supportedCharts.map(c => (
                    <button
                      key={c}
                      onClick={() => { onChangeChartType?.(widget.id, c); setMenuOpen(false); }}
                      style={{
                        ...menuItem,
                        background: widget.chart_type === c ? 'var(--primary)' : 'transparent',
                        color: widget.chart_type === c ? '#fff' : 'var(--text)',
                      }}
                    >
                      {chartLabel(c)}
                    </button>
...
// ─────────────────────────────────────────────────────────────────
// Fetcher dispatch — maps widget_type to the right API call.
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// Body renderer — dispatches on widget_type + chart_type.
// Each branch shapes the API response into the form the Recharts
// component wants, then renders it.
// ─────────────────────────────────────────────────────────────────
function WidgetBody({ widget, data }: { widget: WidgetInstance; data: unknown }) {
  ...
  switch (widget.widget_type) {
    ...
    case 'time_to_first_touch': {
      ...
      if (chart === 'number') {
        return <NumberTile main={`${d.avg_minutes}m`} sub={`SLA breach ${d.sla_breach_pct}% (${d.total} leads)`}/>;
      }
    ...
    case 'stuck_leads': {
      ...
      if (chart === 'bar') {
        return <BarSeries data={[
          { name: '7d', count: d.count_7d }, { name: '14d', count: d.count_14d }, { name: '30d', count: d.count_30d },
        ]} keys={['count']} />;
      }
    ...
    case 'disqualification_reasons': {
      ...
      if (chart === 'pie' || chart === 'donut') {
        return <PieSeries data={rows.map(r => ({ name: r.reason, value: r.count }))} donut={chart === 'donut'} />;
      }
      if (chart === 'horizontal-bar') {
        return <HBarSeries data={rows.map(r => ({ name: r.reason, value: r.count }))} />;
      }
      if (chart === 'table') {
        return <TableView rows={rows.map(r => ({ Reason: r.reason, Count: r.count }))} />;
      }
    ...
    case 'stage_conversion': {
      ...
      if (chart === 'table') {
        return <TableView rows={rows.map(r => ({
          From: r.from_stage, To: r.to_stage, Entered: r.entered, Advanced: r.advanced, 'Rate %': r.rate,
        }))} />;
      }
      const series = rows.map(r => ({ name: `${r.from_stage} → ${r.to_stage}`, rate: r.rate }));
      return chart === 'horizontal-bar'
        ? <HBarSeries data={series.map(s => ({ name: s.name, value: s.rate }))} valueLabel="%" />
        : <BarSeries data={series} keys={['rate']} suffix="%" />;
    }
    ...
    case 'touchpoints_to_response': {
      ...
      if (chart === 'pie' || chart === 'donut') {
        return <PieSeries data={rows.map(r => ({ name: r.bucket, value: r.count }))} donut={chart === 'donut'} />;
      }
      if (chart === 'horizontal-bar') {
        return <HBarSeries data={rows.map(r => ({ name: r.bucket, value: r.count }))} />;
      }
    ...
    case 'cohort_conversion': {
      ...
      // Always render as a heatmap table — Recharts has no built-in cohort matrix.
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.cohort_month}>
                  ...
                  {r.cells.map(c => (
                    <td key={c.age_months} style={{
                      ...tdStyle,
                      background: heatColor(c.rate),
                      color: c.rate > 30 ? '#fff' : 'var(--text)',
                      textAlign: 'center',
                    }}>{c.rate}%</td>
                  ))}
                </tr>
    ...
    case 'engagement_comparison': {
      ...
      if (chart === 'number') {
        return <NumberTile main={`${d.won.avg} vs ${d.lost.avg}`} sub={`Avg touches: won vs lost (${d.won.count} won, ${d.lost.count} lost)`} />;
      }
      return <BarSeries data={[
        { name: 'Won', avg_touches: d.won.avg }, { name: 'Lost', avg_touches: d.lost.avg },
      ]} keys={['avg_touches']} />;
    }
    ...
    case 'score_band_conversion': {
      ...
      if (chart === 'table') {
        return <TableView rows={rows.map(r => ({ Band: r.band, Total: r.total, Converted: r.converted, 'Rate %': r.rate }))} />;
      }
      return chart === 'line'
        ? <LineSeries data={rows.map(r => ({ name: r.band, rate: r.rate }))} keys={['rate']} />
        : <BarSeries data={rows.map(r => ({ name: r.band, rate: r.rate }))} keys={['rate']} suffix="%" />;
    }
    ...
    case 'territory_conversion': {
      ...
      if (chart === 'horizontal-bar') {
        return <HBarSeries data={rows.map(r => ({ name: r.territory, value: r.rate }))} valueLabel="%" />;
      }
  ...
  return (
    <div style={{ overflow: 'auto', flex: 1 }}>
      <table style={tableStyle}>
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i}>{cols.map(c => <td key={c} style={tdStyle}>{r[c]}</td>)}</tr>
          ))}
        </tbody>
      </table>
    </div>
...
function Tile({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={card}>
      <div style={header}><div style={title$}>{title}</div></div>
...
function chartLabel(c: ChartType): string {
  return ({ number: 'Number', bar: 'Bar', line: 'Line', area: 'Area', pie: 'Pie', donut: 'Donut',
    'horizontal-bar': 'H-Bar', table: 'Table', heatmap: 'Heatmap' } as Record<ChartType, string>)[c];
}
...
const title$: React.CSSProperties = title;
