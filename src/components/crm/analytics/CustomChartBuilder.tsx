 ...
 *
 * Users pick:
 *   - a data source (one of the analytics datasets in CUSTOM_DATASETS)
 *   - an X axis field
 *   - a Y axis field (auto-filtered to numeric for non-table charts)
 *   - a chart type (bar / line / area / pie / donut / horizontal-bar / table)
 *   - a title for the saved chart
 *
 * On Save, hands a fully-shaped WidgetInstance (widget_type='custom',
 * chart_type, config:{data_source,x_field,y_field,label}) back to the
 * parent for insertion or in-place update.
 */
import { useEffect, useMemo, useState } from 'react';
import { X, BarChart3, LineChart as LineIcon, PieChart, Activity, Table as TableIcon } from 'lucide-react';
import {
  CUSTOM_DATASETS, datasetById,
  type ChartType,
} from '../../../lib/crm/widgetCatalog';
...
export default function CustomChartBuilder({ onSave, onClose, initial }: Props) {
  const initialConfig = (initial?.config as CustomConfig | undefined) ?? {};
  const initialDsId = initialConfig.data_source ?? CUSTOM_DATASETS[0].id;

  const [datasetId, setDatasetId] = useState<string>(initialDsId);
  const ds = useMemo(() => datasetById(datasetId) ?? CUSTOM_DATASETS[0], [datasetId]);
  ...
  // When the user picks a different dataset, snap axes + label to the
  // new dataset's defaults so the form is always in a valid state.
  const changeDataset = (id: string) => {
    setDatasetId(id);
    const next = datasetById(id);
    if (next) {
  ...
  // If editing, lock in once on mount so we don't snap defaults over the
      ...
      <div onClick={(e) => e.stopPropagation()} style={dialog}>
        <header style={headerStyle}>
          <div>
            ...
            <div style={titleStyle}>Custom analytics chart</div>
        ...
        <div style={form}>
          <Field label="Chart title">
            <input
              ...
              placeholder="e.g. Lost reasons — Q4"
              style={input}
            />
          </Field>
