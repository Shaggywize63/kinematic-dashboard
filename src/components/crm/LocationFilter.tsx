...
export default function LocationFilter() {
  ...
  return (
    <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
      <select value={state ?? ''} onChange={(e) => setState(e.target.value || null)} style={selectStyle} title="Filter by state">
        <option value="">All states</option>
        {states.map((s) => <option key={s.id} value={s.name}>{s.name}</option>)}
      </select>
      <select value={city ?? ''} onChange={(e) => setCity(e.target.value || null)} disabled={!state || loadingCities} style={{ ...selectStyle, opacity: !state ? 0.5 : 1 }} title="Filter by city">
        <option value="">{state ? 'All cities' : 'Pick a state…'}</option>
        {cities.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
      </select>
      {active && (
        <button type="button" onClick={clear} style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-dim)', padding: '6px 10px', borderRadius: 6, fontSize: 11, cursor: 'pointer' }}>Clear</button>
      )}
    </div>
  );
}
...
const selectStyle: React.CSSProperties = {
  ...
  padding: '7px 10px', borderRadius: 8, fontSize: 12, cursor: 'pointer', minWidth: 120,
};
