...
export default function LeadEditModal({ lead, open, onClose, onSaved }: Props) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Edit Lead"
      ...
      {!form.is_b2c ? (<><SL>Business Details</SL><Grid><F label="Company" value={form.company} onChange={(v) => setForm({ ...form, company: v })} /><F label="Job Title" value={form.title} onChange={(v) => setForm({ ...form, title: v })} /><F label="Industry" value={form.industry} onChange={(v) => setForm({ ...form, industry: v })} /></Grid></>) : (
